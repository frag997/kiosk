...
def background_thread():
    socketio.sleep(10)
    socketio.emit('my_response', {'data': 'Server generated event'}, namespace='/gpt3')
    while True:
        socketio.sleep(60)


@socketio.on('completion_request', namespace='/gpt3')
...
@socketio.on('connect', namespace='/gpt3')
def test_connect():
    global thread
    with thread_lock:
        if thread is None:
...
@socketio.on('disconnect', namespace='/gpt3')
def test_disconnect():
